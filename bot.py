    ...
    async def start(self):
        ...
        if FORCE_SUB_CHANNEL:
            try:
                ...
                self.LOGGER(__name__).info(
                    "Bot Berhenti. pc https://t.me/databaselugh untuk Bantuan"
                )
                sys.exit()

        if FORCE_SUB_CHANNEL_2:
            try:
                link = (await self.get_chat(FORCE_SUB_GROUP_2)).invite_link
                if not link:
        ...
        self.LOGGER(__name__).info(
            f"[ BERHASIL AKTIF ]\n\nBOT Dibuat oleh @{OWNER}\nJika @{OWNER} Membutuhkan Bantuan, Silahkan Tanyakan PC https://t.me/databaselugh
        )
